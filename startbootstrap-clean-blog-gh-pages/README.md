...
Set up an Express application with TypeScript and Nunjucks. Use the [Clean Blog](https://startbootstrap.com/previews/clean-blog) theme from Start Bootstrap as the visual template. Download it from the [theme page](https://startbootstrap.com/theme/clean-blog).

The blog content is provided as JSON below. Store it in a file and read it at startup.

Make sure to download the images before starting.

#### Home page

List all posts on the home page using the index layout. Each entry shows the title, teaser, author, and date. The createdAt field is a Unix timestamp — convert it to a human-readable date string before passing it to the template. Do not modify the JSON structure.

#### Post detail page

Render each post on its own page using the post layout. The URL should include a slug derived from the post title so that each post has a unique, readable path. Display the full content and the post’s image.

#### Contact page

Implement the contact page. The form does not need to process submissions — plain HTML is sufficient. Use a macro for the form fields to avoid repeating input markup. The input should accept a label, placeholder and type parameter. If the type is “textarea”, a textarea element should be rendered instead of an input element.
...
- colorful-umbrella.jpg - https://images.unsplash.com/photo-1535911062114-764574491173
- flowers.jpg - https://images.unsplash.com/photo-1604400247036-e0b38afce25c
- sailing.jpg - https://images.unsplash.com/photo-1499692190251-36a18d26e287